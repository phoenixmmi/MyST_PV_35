...
#define DEL "\n---------------------------------------\n";
#define LIC_PLATE_INP	std::wcout<<"Ââåäèòå íîìåð àâòîìîáèëÿ: ";		std::wstring licensePlate;	std::getline(std::wcin, licensePlate);
#define VIO_INP			std::wcout<<"Ââåäèòå ïðàâîíàðóøåíèå: ";			std::wstring violation;		std::getline(std::wcin, violation); 
...
void printFullMap(const std::map<std::wstring, std::list<std::wstring>>& map)
{
	...
	EventList.push_back(L"Áàçà äàííûõ íàïå÷àòàíà");
		...
		if (m_it.first == licensePlate)
		{
			...
			if (m_it.second.empty()) std::wcout << L"\t...empty...";
				...
				for (std::wstring l_it : m_it.second) {
					std::wcout << "\t" << l_it << L"\n";
					notFound = false;
					EventList.push_back(L"Íàïå÷àòàíû ïðàâîíàðóøåíèÿ ïî íîìåðó: " + licensePlate);
				}
			}
			break;
		}
...
std::map<std::wstring, std::list<std::wstring>> init()
{
	std::map<std::wstring, std::list<std::wstring>> PoliceDB =
	{
		...
		(L"BI0000BI", {L"ïðåâûøåíèå ñêîðîñòè", L"âîçäåíèå â íåòðåçâîì ñîñòîÿíèè"}),

		std::pair<std::wstring, std::list<std::wstring>>
		(L"BI0001BI", {L"åçäà ïî âñòðå÷êå"}),

		std::pair<std::wstring, std::list<std::wstring>>
		(L"BI0002BI", {L"ïàðêîâêà à íåïîëîæåíîì ìåñòå"}),

		std::pair<std::wstring, std::list<std::wstring>>
		(L"BI0003BI", {L"ïðîåçä íà êðàñíûé", L"ïëþíóë â ïîëèöåéñêîãî"})
	};
	EventList.push_back(L"Áàçà äàííûé ïðîèíèöèàëèçèðîâàíà");
	return PoliceDB;
}
void insert(std::map<std::wstring, std::list<std::wstring>>& map)
{
	LIC_PLATE_INP
		if (licensePlate.size() < 6) { std::wcout << L"\n\nÂû íå óêàçàëè íîìåð àâòîìîáèëÿ."; }
		else {
			VIO_INP
				if (violation.size() < 10) { std::wcout << L"\n\nÂû íå óêàçàëè ïðàâîíàðóøåíèå."; }
				else {
					...
					if (offender != map.end())offender->second.push_back(violation);
						...
						EventList.push_back(L"Äîáàâëåí íîâûé íîìåð òðàíñïîðòíîãî ñðåäñòâà");
						std::wcout << L"\n\nÄîáàâëåí íîâûé íîìåð òðàíñïîðòíîãî ñðåäñòâà.";
					}
				}
		}
...
void save(const std::map<std::wstring, std::list<std::wstring>>& map)
{
	...
	fout.close();
	EventList.push_back(L"Áàççà äàííûõ ñîõðàíåíà");
...
void load(std::map<std::wstring, std::list<std::wstring>>& map)
{
	...
	if (fin.is_open())
	{
		...
		EventList.push_back(L"Áàçà äàííûõ çàãðóæåíà");
		std::wcout << L"Database is loaded. \n\n";
	}
	else {
		EventList.push_back(L"Ôàéë çàãðóçêè íå íàéäåí");
...
void printEL()
{
	...
	EventList.push_back(L"Æóðíàë ñîáûòèé ïðîñìîòðåí");
...
void loadEL()
{
	...
	if (fin.is_open())
	{
		...
		EventList.push_back(L"Æóðíàë ñîáûòèé çàãðóæåí");
		std::wcout << L"\n\nÆóðíàë ñîáûòèé çàãðóæåí";
	}
	else {
		EventList.push_back(L"Ôàéë çàãðóçêè íå íàéäåí");
		std::wcout << L"Error: file is not found";
...
void saveEL()
{
	...
	fout.close();
	EventList.push_back(L"Æóðíàë ñîáûòèé ñîõðàíåí");
	std::wcout << L"\n\nÆóðíàë ñîáûòèé ñîõðàíåí";
	std::wcout << L"\n\n\t[Esc] Exit\t\t\t[Any key] Resave\n";
}